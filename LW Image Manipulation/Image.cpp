...
Image::Image(const Image& element) : image(nullptr), width(0), height(0) 
{
    allocateImage(element.width, element.height);

    for (unsigned int i = 0; i < width; i++)
     {
        for (unsigned int j = 0; j < height; j++) 
        {
            image[i][j] = element.image[i][j];
        }
    }
}

Image& Image::operator=(const Image& element) 
{
    if (this != &element) {
